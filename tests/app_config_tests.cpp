...
TEST_CASE("loadAppConfig reads views and keybinding overrides from TOML") {
    ...
    CHECK(config.theme_overrides.watch_error == ftxui::Color::RGB(0xff, 0x11, 0x22));
    CHECK(config.theme_overrides.selected_watch_error == ftxui::Color::RGB(0xee, 0x33, 0x44));
    ...
    CHECK(config.theme_overrides.memory_highlight_ascii_background == ftxui::Color::RGB(0x20, 0x30, 0x40));
    CHECK(config.theme_overrides.selected_memory_highlight_hex == ftxui::Color::RGB(0xfe, 0xdc, 0xba));
    ...
    const auto& launch_profile = config.launch_profiles[0];
    CHECK(launch_profile.name == "tests");
    CHECK(launch_profile.program == "/tmp/sample-tests");
    const auto profile_arguments = launch_profile.arguments.value_or(std::vector<std::string>{});
    REQUIRE(profile_arguments.size() == 2);
    CHECK(profile_arguments[0] == "--suite");
    CHECK(profile_arguments[1] == "unit");
    const auto profile_watches = launch_profile.watches.value_or(std::vector<std::string>{});
    REQUIRE(profile_watches.size() == 2);
    CHECK(profile_watches[0] == "test_value");
    CHECK(profile_watches[1] == "test_ptr->field");
    const auto profile_breakpoints = launch_profile.breakpoints.value_or(std::vector<SSourceBreakpointConfig>{});
    REQUIRE(profile_breakpoints.size() == 1);
    CHECK(profile_breakpoints[0].source_path == std::filesystem::path("/tmp/tests.cpp"));
    CHECK(profile_breakpoints[0].line == 17);
...
TEST_CASE("loadAppConfigWithBaseConfig preserves user UI config while overlaying launch config") {
    ...
    CHECK(result.config.theme_overrides.memory_hex == ftxui::Color::RGB(0x12, 0x34, 0x56));
    CHECK(result.config.session_mode == eSessionMode::DAP_LAUNCH);
