...
void executeCommand(eCommand command, eFocusPane& focused_pane, SViewVisibilityState& view_visibility) {
    switch (command) {
        ...
        case eCommand::CHOOSE_PROFILE: break;
...
std::optional<eCommand> parseCommandName(const std::string& command_name) {
    ...
    if (command_name == "toggle_breakpoint") {
        return eCommand::TOGGLE_BREAKPOINT;
    }
    if (command_name == "set_memory_target") {
        return eCommand::SET_MEMORY_TARGET;
    }
    if (command_name == "continue_execution") {
        return eCommand::CONTINUE_EXECUTION;
    }
    if (command_name == "step_over") {
        return eCommand::STEP_OVER;
    }
    if (command_name == "step_into") {
        return eCommand::STEP_INTO;
    }
    ...
    if (command_name == "choose_profile") {
        return eCommand::CHOOSE_PROFILE;
    }
...
std::string commandDescription(eCommand command) {
    switch (command) {
        ...
        case eCommand::CHOOSE_PROFILE: return "Choose Profile";
