...
void executeCommand(eCommand command, eFocusPane& focused_pane, SViewVisibilityState& view_visibility) {
    switch (command) {
        ...
        case eCommand::ATTACH_PROCESS: break;
...
std::optional<eCommand> parseCommandName(const std::string& command_name) {
    ...
    if (command_name == "attach_process") {
        return eCommand::ATTACH_PROCESS;
    }
...
std::string commandDescription(eCommand command) {
    switch (command) {
        ...
        case eCommand::ATTACH_PROCESS: return "Attach to Process";
...
std::vector<SKeybinding> defaultKeybindings() {
    return {
        ...
        {.keys = "Space p", .command = eCommand::ATTACH_PROCESS},     {.keys = "Space c", .command = eCommand::CYCLE_THEME},
