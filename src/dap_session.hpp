...
struct SDapStepOverRequest {
    int thread_id = 0;
};

struct SDapStepOverResponse {
    bool        success = false;
    std::string error_message;
};

struct SDapStepIntoRequest {
    int thread_id = 0;
};

struct SDapStepIntoResponse {
    bool        success = false;
    std::string error_message;
};
...
class CDapDebugSession : public IDebugSession {
  public:
    ...
    static std::string                   buildStepOverRequestMessage(int sequence_number, const SDapStepOverRequest& step_over_request);
