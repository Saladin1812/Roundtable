...
class IMemoryDataProvider {
  public:
    virtual ~IMemoryDataProvider() = default;

    virtual SMemoryReadResult readMemory(const SMemoryReadRequest& request) const = 0;
};

class CMockMemoryDataProvider : public IMemoryDataProvider {
  public:
    SMemoryReadResult readMemory(const SMemoryReadRequest& request) const override;
};
